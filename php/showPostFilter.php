<?php 
	...
	<div style='padding:10px'>
       	<div class='valign-wrapper'>
       		<div style='background:url({$photoURLComment}); background-size:100%; background-position:center; display:inline-block; border-radius:40px; width:30px; height:30px; margin 0 20;'></div> <b>{$displayNameComment}</b>: {$textComment}
       	</div>
    </div>
	...
	$consVBF = 'SELECT * FROM profiles WHERE bf LIKE "%'.$row["uid"].'%" AND uid = "'.$uid.'" ';
	$resultVBF = $conn->query($consVBF);

	if ($resultVBF->num_rows > 0) {
